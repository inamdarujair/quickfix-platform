    ...
    <div>
      <div className="mb-6 flex justify-end">
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger className="border-input bg-transparent text-foreground sm:w-48">
            <SelectValue />
          </SelectTrigger>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {bookings.map((b) => (
            <div key={b.id} className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-heading text-base font-medium text-foreground">{b.service_title}</h3>
                  <StatusBadge status={b.status} />
                </div>
                <p className="mt-1 text-sm text-muted-foreground">for {b.customer_name}</p>
                <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground/80">${b.price}</span>
                </div>
              </div>
              <div className="flex gap-2">
                {b.status === "pending" && (
                  <>
                    ...
                    <Button size="sm" variant="outline" onClick={() => updateStatus(b.id, "rejected")} data-testid={PROVIDER_BOOKING_ROW.rejectButton(b.id)} className="border-border bg-transparent text-red-400 hover:bg-red-500/10">
